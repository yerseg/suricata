...
/*
 * TODO: Update \author in this file and in output-json-s7comm.h.
 ...
 *
 * Implement JSON/eve logging app-layer S7comm.
 */
    ...
    const Packet *p, Flow *f, void *state, void *tx, uint64_t tx_id)
{
    ...
    JsonBuilder *js = CreateEveHeader(p, LOG_DIR_PACKET, "s7comm", NULL);
    if (unlikely(js == NULL)) {
        return TM_ECODE_FAILED;
    }

    jb_open_object(js, "s7comm");

    /* Log the request buffer. */
    if (s7commtx->request_buffer != NULL) {
        jb_set_string_from_bytes(js, "request", s7commtx->request_buffer,
                s7commtx->request_buffer_len);
    }

    /* Log the response buffer. */
    if (s7commtx->response_buffer != NULL) {
        jb_set_string_from_bytes(js, "response", s7commtx->response_buffer,
...
static TmEcode JsonS7commLogThreadInit(ThreadVars *t, const void *initdata, void **data)
{
    ...
    if (initdata == NULL) {
        SCLogDebug("Error getting context for EveLogS7comm.  \"initdata\" is NULL.");
        goto error_exit;
    }
...
void JsonS7commLogRegister(void)
{
    ...
    OutputRegisterTxSubModule(LOGGER_JSON_S7COMM, "eve-log", "JsonS7commLog",
        "eve-log.s7comm", OutputS7commLogInitSub, ALPROTO_S7COMM,
        JsonS7commLogger, JsonS7commLogThreadInit,
        JsonS7commLogThreadDeinit, NULL);
